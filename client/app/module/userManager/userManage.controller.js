(function () {
    ...
    angular.module('app.table')
        .controller('UserManageCtrl', ['$scope', '$filter','kendoConfig', '$api','$compile', '$mdToast','$mdDialog','$constant', TableCtrl]);

    function TableCtrl($scope, $filter,kendoConfig, $api,$compile,$mdToast,$mdDialog,$constant) {
        var init;
        $scope.appForm ={};
        ...
        //tìm kiếm phòng ban
        $scope.queryOrganize = function(searchText){
        ...
        $scope.selectedItemChange=function(item){
            ...
            if(item!=null&& item.id>0){
                $scope.searchForm.organizeid=item.id;
                console.log($scope.searchForm.organizeid);
            }
        }

        $scope.showDepartmentSearchForm=function(ev){
            console.log(ev);
            $mdDialog.show({
                controller: DialogController,
                templateUrl: 'app/tbl/selectDepartment.html',
